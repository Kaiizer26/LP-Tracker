...
router.get('/', async (req, res) => {
    try {
        ...
        runeparticipants ? res.status(200).json(runeparticipants) : res.status(404).json({ message: "Not found" });
    } catch (error) {
...
router.get('/:rune_participant_id', async (req, res) => {
    try {
        const runeparticipant = await RuneParticipant.getRuneParticipantById(req.params.rune_participant_id);
        runeparticipant ? res.status(200).json(runeparticipant) : res.status(404).json({ message: "Not found" });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});
...
router.post('/', async (req, res) => {
    try {
        const newRuneParticipant = await RuneParticipant.createRuneParticipant(req.body);
...
router.put('/:rune_participant_id', async (req, res) => {
    try {
        const updatedRuneParticipant = await RuneParticipant.updateRuneParticipant(req.params.rune_participant_id, req.body);
...
router.patch('/:rune_participant_id', async (req, res) => {
    try {
        const existingRuneParticipant = await RuneParticipant.getRuneParticipantById(req.params.rune_participant_id);
        ...
        const updatedRuneParticipant= await RuneParticipant.updateRuneParticipant(req.params.rune_participant_id, {
